...
class Settings(BaseModel):
    ...
    cors_allowed_methods: list[str] = Field(default_factory=lambda: os.getenv("CORS_ALLOWED_METHODS", "*").split(","))
    cors_allowed_headers: list[str] = Field(default_factory=lambda: os.getenv("CORS_ALLOWED_HEADERS", "*").split(","))
    cors_allow_credentials: bool = Field(default_factory=lambda: os.getenv("CORS_ALLOW_CREDENTIALS", "false").lower() in ("1", "true", "yes"))
    cors_expose_headers: list[str] = Field(default_factory=lambda: [h for h in os.getenv("CORS_EXPOSE_HEADERS", "").split(",") if h])
